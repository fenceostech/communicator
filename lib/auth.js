...
// Guest sessions are shorter-lived: once they expire, the guest is asked for
// their name again on their next visit.
export const GUEST_TTL_SECONDS = 12 * 60 * 60
...
// A name-only guest session. No DB user is created; the role 'guest' is
// carried in the signed cookie and enforced by requireRole server-side
// (guests are non-admin, so every admin-only mutation rejects them).
export async function createGuestSessionToken({ name, sid }) {
  return signSession(
    {
      uid: null,
      email: null,
      role: 'guest',
      name,
      initials: initialsFor(name) || 'G',
      guest: true,
      sid,
      iat: Date.now(),
      exp: Date.now() + GUEST_TTL_SECONDS * 1000,
    },
    secret()
  )
}
...
export async function setSessionCookie(token, maxAge) {
  ...
  store.set(SESSION_COOKIE, token, maxAge == null ? cookieOptions() : cookieOptions(maxAge))
}
